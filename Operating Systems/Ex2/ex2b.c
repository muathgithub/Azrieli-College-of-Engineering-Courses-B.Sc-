// Program Name: Mini Shell Version B
...
// this version (Version B) also prints the sched for each son process that made by fork
...
void printSched(pid_t *, int *, char**, const int*);
...
int main() {
    ...
    for (;;) {
        ...
        // then it continue to execute the given user command
            ...
            printSched(&pid, &isInWord, commandArgs, &argsArrayLen);
...
// this function prints the sched of this process
// it receives four pointers:
// 1-) pid in order to save the sons pid
// 2-) the flag isInWord which it passes to other needed functions to prepare the cat command
// 3-) commandArgs to free all the fathers manual allocated memories
// 4-) argsArrayLen which the freeAll() needs to free the fathers manual allocated memories
void printSched(pid_t *pid, int *isInWord, char** commandArgs, const int* argsArrayLen) {
    // a char array for holding the string of the cat command
    char catCommandSrt[MAX_CAT_LEN];
    // a double char pointer for pointing on the first address of the command arguments pointers (As Array of char*)
    char **catCommandArgs;

    // filling the catCommandSrt with the proper cat command to print the process sched according to it's id
    snprintf(catCommandSrt, MAX_CAT_LEN, "cat /proc/%d/sched", getpid());

    // calculating the length of the catCommand after removing the '\n' character if it exists
    // by calling the function strLenNoNewLine()
    int catCommandStrLen = strLenNoNewLine(catCommandSrt);

    // calculating the number of the arguments/words (int the sentence) + NULL (at the end)
    int catArgsNum = argsCount(catCommandSrt, isInWord, &catCommandStrLen) + 1;

    // converting the sentence to array of arguments for the use of execvp()
    catCommandArgs = toArgsArray(catCommandSrt, isInWord, &catCommandStrLen, &catArgsNum);

    // forking a new process to execute the cat command
    *pid = fork();

    // checking if the process is the son process
    // if true free the manual allocated memory that came from the father
    // then execute the cat command to print the fathers sched
    if (*pid == 0) {
        freeAll(commandArgs, argsArrayLen);
        int status = execvp(catCommandArgs[0], catCommandArgs);
        // checking if the execvp() failed
        // if true print a proper message, free all the manual allocated memories and exit/end the program
        if (status == -1) {
            perror("Command Not Found: ");
            freeAll(catCommandArgs, &catArgsNum);
            exit(EXIT_FAILURE);
        }
    }
    // this else means that the process is the father(first/main process)
    // if true it waits till the son finishes then it will free all the cat command manual allocated memories then
    else
    {
        wait(NULL);
        freeAll(catCommandArgs, &catArgsNum);
